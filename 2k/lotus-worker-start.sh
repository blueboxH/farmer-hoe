...
then
    nohup env FIL_PROOFS_PARENT_CACHE=${LOTUS_PATH_WORKER_2K} FIL_PROOFS_USE_GPU_TREE_BUILDER=1 FIL_PROOFS_USE_GPU_COLUMN_BUILDER=1 FIL_PROOFS_MAXIMIZE_CACHING=1  LOTUS_WORKER_PATH=${LOTUS_PATH_WORKER_2K}/lotusworker FIL_PROOFS_PARAMETER_CACHE=/root/v28 TMPDIR=${LOTUS_PATH_WORKER_2K} RUST_LOG=Info ${LOTUS_PATH_WORKER_BIN}/lotus-worker run --addpiece=true --precommit1=true  --precommit2=true --unseal=false --commit=false --parallel-fetch-limit=15 --listen=$(ifconfig|grep "inet 192.168"|awk '{print $2}'):20827> ${LOTUS_PATH_WORKER_2K}/worker.log 2>&1 &
elif [[ $taskType == Commit ]]
then
    nohup env FIL_PROOFS_PARENT_CACHE=${LOTUS_PATH_WORKER_2K} FIL_PROOFS_USE_GPU_TREE_BUILDER=1 FIL_PROOFS_USE_GPU_COLUMN_BUILDER=1 FIL_PROOFS_MAXIMIZE_CACHING=1  LOTUS_WORKER_PATH=${LOTUS_PATH_WORKER_2K}/lotusworker FIL_PROOFS_PARAMETER_CACHE=/root/v28 TMPDIR=${LOTUS_PATH_WORKER_2K} RUST_LOG=Info ${LOTUS_PATH_WORKER_BIN}/lotus-worker run --addpiece=false --precommit1=false  --precommit2=false --unseal=false --commit=true --parallel-fetch-limit=15 --listen=$(ifconfig|grep "inet 192.168"|awk '{print $2}'):20827> ${LOTUS_PATH_WORKER_2K}/worker.log 2>&1 &
fi
